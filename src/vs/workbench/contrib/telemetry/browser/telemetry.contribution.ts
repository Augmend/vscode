...
type FileTelemetryDataFragment = {
	...
	enablelistedjson?: { classification: 'SystemMetaData', purpose: 'FeatureInsight' };
};

export class TelemetryContribution extends Disposable implements IWorkbenchContribution {

	private static ENABLELIST_JSON = ['package.json', 'package-lock.json', 'tsconfig.json', 'jsconfig.json', 'bower.json', '.eslintrc.json', 'tslint.json', 'composer.json'];
	private static ENABLELIST_WORKSPACE_JSON = ['settings.json', 'extensions.json', 'tasks.json', 'launch.json'];
	...
	private getTypeIfSettings(resource: URI): string {
		...
		for (const folder of folders) {
			if (isEqualOrParent(resource, folder.toResource('.vscode'))) {
				...
				if (TelemetryContribution.ENABLELIST_WORKSPACE_JSON.indexOf(filename) > -1) {
	...
	private getTelemetryData(resource: URI, reason?: number): TelemetryData {
		...
		const telemetryData = {
			...
			path: hash(path),
			reason,
			enablelistedjson: undefined as string | undefined
		};

		if (ext === '.json' && TelemetryContribution.ENABLELIST_JSON.indexOf(fileName) > -1) {
			telemetryData['enablelistedjson'] = fileName;
		}
